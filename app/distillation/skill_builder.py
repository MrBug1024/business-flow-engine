...
# 生成产物（发布前源目录）
    {scenario_id}/skills/
        ...
        manifest.json              ── MCP/验证通道使用的场景元数据
...
本场景是一个 Skill bundle，内部每项能力都是一个包含 `SKILL.md` 的标准子 Skill。
脚本只作为各自子 Skill 的 `scripts/` 资源存在，不再暴露散落的自定义工具目录。

| Skill | 入口 | 功能 | 阶段 |
|------|------|------|------|
| 数据读取 Skill | `skill_data_reader/SKILL.md` → `scripts/skill_data_reader.py` | 读取任意格式业务数据并校验字段 | 上传 |
| 主业务 Skill | `main_skill/SKILL.md` → `scripts/skill_executor.py` | 应用知识条目到业务数据产出结果 | 执行 |
| NL 规则解析 Skill | `skill_nl_rule_parser/SKILL.md` → `scripts/skill_nl_rule_parser.py` | 识别自然语言规则语义模式 | 执行辅助 |
| 知识检索 Skill | `skill_knowledge_search/SKILL.md` → `scripts/search_knowledge.py` / `scripts/list_knowledge.py` | 搜索和浏览知识条目 | 查询 |
| 即席查询 Skill | `skill_query_data/SKILL.md` → `scripts/query_data.py` | 对新业务数据执行 DuckDB SQL 并落盘结果 | 查询 |
...
```bash
# 浏览所有知识条目
python skill_knowledge_search/scripts/list_knowledge.py 50 /path/to/new_data

# 搜索特定条目
python skill_knowledge_search/scripts/search_knowledge.py "关键词" 20 /path/to/new_data
```
...
```bash
python skill_query_data/scripts/query_data.py \\
...
- Pipeline SQL 产出可确定性执行；知识表驱动产出会先返回命中的知识/规则行，最终判断需要宿主 Agent
  结合规则原文和 `skill_query_data/scripts/query_data.py` 现场查询业务数据。
...
def _write_main_skill_md(skill_dir: Path, scenario: Scenario,
    ...
    md = _skill_frontmatter(
        ...
        f"knowledge-driven rule execution, or deterministic scenario processing for {scenario.name} business data.",
    ) + f"""
...
## 上下文资源
- `domain_knowledge.json`：表结构、字段语义和关联关系。
- `output_specs.json`：可执行产出规格。
- `dispatch_config.json`：知识/规则驱动配置；无知识表时为空对象。
"""
...
""",
    ...
    (query_dir / "SKILL.md").write_text(
        _skill_frontmatter(
            ...
            "Use for field checks, joins, aggregations, and rule-by-rule verification after reading the schema.",
        ) + f"""
...
""",
    ...
    (nl_parser_dir / "SKILL.md").write_text(
        _skill_frontmatter(
            ...
            "Use to classify rule patterns, thresholds, entities, and execution hints before querying data.",
        ) + f"""
...
def _write_subagent_system_prompt(
    ...
    required_tables = "、".join(card.get("required_tables") or []) or "无固定表名要求，按 main_skill/domain_knowledge.json 判断"
    tool_names = "、".join(t.get("name", "") for t in tools if t.get("name")) or "见 SKILL.md"
...
附件上传、文件预览、结果文件下载由宿主 Agent 平台负责。不要假设本 Skill 可以直接写宿主沙盒目录。
如果宿主已经把数据整理成可访问目录、表格内容或 DataFrame，再根据
`main_skill/domain_knowledge.json`、`main_skill/output_specs.json` 和
`main_skill/dispatch_config.json` 完成业务判断。
...
2. 先读取能力说明和 schema，再判断需要哪些表、字段、规则和流程节点。
3. 知识/规则驱动任务要先定位规则原文，再结合业务数据逐条判断。
4. 不编造字段、表名、流程节点或业务结论；缺少关键数据时说明缺什么。
5. 输出结果时说明依据、命中规则、关键字段和可复核的数据范围。
"""
