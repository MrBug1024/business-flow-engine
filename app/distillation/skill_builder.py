...
| 即席 SQL 查询 | `scripts/query_data.py` | 对新业务数据执行 DuckDB SQL 并落盘结果 | 查询 |
...
```bash
# 读取并校验数据结构（在本 Skill 根目录执行）
python skill_data_reader/scripts/skill_data_reader.py /path/to/new_data
```

```python
import importlib.util
spec = importlib.util.spec_from_file_location("dr", "skill_data_reader/scripts/skill_data_reader.py")
...
```bash
# 浏览所有知识条目
python utils/scripts/list_knowledge.py 50 /path/to/new_data

# 搜索特定条目
python utils/scripts/search_knowledge.py "关键词" 20 /path/to/new_data
```
...
```bash
# 查看可用产出列表
python main_skill/scripts/skill_executor.py
...
```python
import importlib.util
spec = importlib.util.spec_from_file_location("nlp", "skill_nl_rule_parser/scripts/skill_nl_rule_parser.py")
mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
...
# 伪代码示例
import importlib.util
spec = importlib.util.spec_from_file_location("se", "main_skill/scripts/skill_executor.py")
mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
...
- Pipeline SQL 产出可确定性执行；知识表驱动产出会先返回命中的知识/规则行，最终判断需要宿主 Agent
  结合规则原文和 `scripts/query_data.py` 现场查询业务数据。
- 历史结果表 = 输出列结构格式模板，不约束可执行的知识条目范围
- NL 规则解析（`skill_nl_rule_parser`）辅助提升复杂规则的识别精度，可选使用

---

## 八、作为 Skill 安装到第三方（推荐）

把整个目录复制到宿主的 skills 目录即可，例如 Codex：

```bash
# 目录名见根 SKILL.md 的 frontmatter name
cp -r <本目录> ~/.codex/skills/
```

安装后，宿主会先读取根 `SKILL.md`；只有需要表结构/流程细节时，再读取本文件和 JSON sidecar。

## 九、作为 MCP Server 挂载到第三方（可选）
...
> 远程 MCP 适合平台托管调试；要让第三方彻底脱离平台，请优先采用上面的 Skill 安装方式。
...
def _generate_tool_schema(skill_dir: Path, scenario: Scenario) -> None:
    ...
    schema = {"tools": [execute_tool, search_tool, list_tool, query_tool]}
...
def materialize_skills(scenario: Scenario) -> list[Skill]:
    ...
    # ---- 根级脚本：给 Agent Skill 宿主的稳定入口 ----
    root_scripts = base / "scripts"
    root_scripts.mkdir(parents=True, exist_ok=True)
    (root_scripts / "query_data.py").write_text(_QUERY_DATA_TEMPLATE, encoding="utf-8")
...
""",
    ...
    query_skill = Skill(
        skill_id="skill_query_data",
        name="业务数据即席查询",
        operation="QUERY",
        description="对本场景业务数据执行 DuckDB SQL，支持任意表、字段、JOIN、聚合并落盘结果。",
        is_main=False,
        path=str(root_scripts / "query_data.py"),
        capability="query(sql, data_dir, out_dir) → CSV 结果 + 预览",
    )

    all_skills = [main_skill, utils_skill, query_skill, reader_skill, nl_parser_skill, *node_skills, *evolved]
...
def _write_manifest(
    ...
    for s in skills:
        ...
        elif s.skill_id == "skill_query_data":
            entry["entry_point"] = "scripts/query_data.py"
        elif s.skill_id == "skill_data_reader":
            entry["entry_point"] = "skill_data_reader/scripts/skill_data_reader.py"
        elif s.skill_id == "skill_nl_rule_parser":
    ...
    manifest = {
        ...
        "entry_points": {
            ...
            "query_data": "scripts/query_data.py",
...
def _synthesize_capability_card(
    ...
    return {
        ...
        "primary_install_mode": "agent_skill",
    }
...
def _write_mcp_descriptor(
    ...
    mcp_doc = {
        ...
        **card,
        ...
        "agent_skill": {
            "name": skill_name,
            "install": "将整个能力包目录复制到宿主的 skills 目录即可。Codex 示例：~/.codex/skills/" + skill_name,
            "entry": "SKILL.md",
            "primary": True,
        },
        "tools": tools,
    ...
    # ---- 根 SKILL.md：Agent Skill 兼容（复制目录即安装；MCP 只是可选增强）----
    _write_openai_agent_metadata(skill_dir, skill_name, card)
    when_summary = "；".join(card["when_to_use"])
    not_summary = "；".join(card["not_for"])
    desc_line = (
        f"{card['summary']} Use when Codex or another AI agent needs this business scenario capability: "
        f"{when_summary}. Do NOT use for: {not_summary}."
    )
    # frontmatter 单行 description，避免换行破坏 YAML
    desc_line = desc_line.replace("\n", " ").replace('"', "'")
    tool_lines = "\n".join(f"- `{t['name']}` — {t['description']}" for t in tools)
    reasoning_note = (
        "本场景是知识表驱动模式：`execute` 负责筛出知识/规则行，最终业务判断需要你读取规则原文，"
        "再用 `scripts/query_data.py` 现场构造 SQL 落地查询。它不依赖生成平台，但依赖宿主 AI 的推理。"
        if card.get("requires_host_llm_reasoning")
        else "本场景是 Pipeline 模式：可直接用 `main_skill/scripts/skill_executor.py` 确定性执行产出。"
    )
    skill_md = f"""---
name: {skill_name}
...
## 快速开始
1. 将整个目录作为一个 Skill 安装，例如复制到 `~/.codex/skills/{skill_name}`。
2. 用户给出业务数据目录时，先运行 `python skill_data_reader/scripts/skill_data_reader.py <data_dir>` 校验表结构。
3. 需要精确表结构、字段语义、ER、流程或输出规格时，再读取 `SCENARIO_CONTEXT.md`、`main_skill/domain_knowledge.json`、`manifest.json`。
4. 搜索知识条目：`python utils/scripts/search_knowledge.py "关键词" 20 <data_dir>`。
5. 执行产出或筛选知识行：`python main_skill/scripts/skill_executor.py <output_id> <data_dir> <out_dir> '<params>'`。
6. 任意 SQL 查询：`python scripts/query_data.py --data-dir <data_dir> --sql 'SELECT * FROM "表名" LIMIT 10'`。
