...
`derive-business-flow` 只在 `discover-data-relations` 已完成且交付物通过验收后运行。它不会读取原始业务数据，也不会把上游主数据路径直接当作流程。模型负责在场景级粒度综合稳定业务责任；包内脚本负责校验上游指纹、流程粒度、证据引用、状态、控制、分支、覆盖和历史验证边界，并确定性生成交付物。

典型交付物位于 `outputs/business-flow/`：

- `business-flow.json`：供后续能力蒸馏使用的结构化宏观流程。
- `business-flow.mmd`：主流程、分支、状态和控制的 Mermaid 图。
- `business-flow-report.md`：逐阶段推断依据、置信度、验证计划和边界。
- `flow-claims.json`：通过校验的正式流程主张。
- `flow-brief.json` 与 `prepare-status.json`：有界综合简报和前置门槛检查点。
...
默认后端地址为 `http://127.0.0.1:8089`，接口文档为 `http://127.0.0.1:8089/docs`。
