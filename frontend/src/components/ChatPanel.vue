<template>
  <div class="chat">
    <header class="chat-head">
      ...
      <div class="spacer" />
      <el-button v-if="clearPath" text size="small" :icon="Delete" @click="clearHistory">清空</el-button>
    </header>
      ...
      <template v-for="(m, i) in messages" :key="i">
        ...
        <div v-else class="row ai">
          ...
          <div class="msg-group">
            ...
            <div v-if="m.content" class="bubble ai" v-html="render(m.content)" />
          </div>
        </div>
      </template>
    ...
    <div v-if="quicks && quicks.length && !pendingInteraction" class="quicks">
      <button v-for="q in quicks" :key="q.text" class="quick" @click="send(q.text)">
    ...
    <div class="composer">
      <div class="composer-box" :class="{ focused }">
        ...
        <div class="composer-foot">
          ...
          <el-button v-if="!streaming" type="primary" class="send" :icon="Promotion" :disabled="!draft.trim()" @click="send()">发送</el-button>
          <el-button v-else type="danger" class="send" :icon="VideoPause" @click="stop">停止</el-button>
        </div>
      </div>
    </div>
  </div>
...
async function loadHistory() {
  if (!props.historyPath) { messages.value = []; return }
  try {
...
.chat { height: 100%; display: flex; flex-direction: column; background: var(--bg-app); background-image: var(--bg-app-grad); }
...
.chat-ico { color: var(--brand); font-size: 17px; }
.chat-title { font-size: var(--text-base); font-weight: 700; color: var(--text-1); }
.spacer { flex: 1; }

/* Messages ---------------------------------------------------------------- */
.messages { flex: 1; overflow-y: auto; padding: 20px 18px; display: flex; flex-direction: column; gap: 18px; }

.empty { margin: auto; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 20px; }
.empty-orb { margin-bottom: 8px; opacity: 0.95; }
.empty-title { font-size: var(--text-lg); font-weight: 700; color: var(--text-1); }
.empty-sub { font-size: var(--text-base); color: var(--text-3); max-width: 320px; line-height: 1.65; }
...
.bubble { border-radius: var(--r-lg); padding: 11px 15px; font-size: var(--text-md); line-height: 1.65; word-break: break-word; }
.bubble.user {
  max-width: 82%;
...
.msg-group { display: flex; flex-direction: column; gap: 8px; max-width: 88%; min-width: 0; }
.bubble.ai {
...
.bubble.ai :deep(p) { margin: 0 0 8px; }
.bubble.ai :deep(p:last-child) { margin-bottom: 0; }
.bubble.ai :deep(h1), .bubble.ai :deep(h2), .bubble.ai :deep(h3) { font-size: var(--text-md); margin: 12px 0 6px; }
.bubble.ai :deep(ul), .bubble.ai :deep(ol) { margin: 4px 0; padding-left: 20px; }
.bubble.ai :deep(li) { margin: 2px 0; }
.bubble.ai :deep(table) { border-collapse: collapse; font-size: var(--text-sm); display: block; overflow-x: auto; margin: 6px 0; }
.bubble.ai :deep(th), .bubble.ai :deep(td) { border: 1px solid var(--border); padding: 5px 9px; }
.bubble.ai :deep(th) { background: var(--surface-sunken); }
.bubble.ai :deep(code) { background: var(--code-bg); padding: 1.5px 5px; border-radius: 5px; font-family: var(--font-mono); font-size: var(--text-sm); }
.bubble.ai :deep(pre) { background: var(--code-bg); border: 1px solid var(--border); padding: 12px; border-radius: var(--r-sm); overflow-x: auto; }
.bubble.ai :deep(pre code) { background: none; padding: 0; }
.bubble.ai :deep(a) { color: var(--brand); }
...
.live-ok { color: var(--success); }
.status-line { display: flex; align-items: center; gap: 9px; font-size: var(--text-sm); color: var(--warning); font-weight: 500; }
.spinner { width: 13px; height: 13px; border-radius: 50%; border: 2px solid var(--warning); border-top-color: transparent; animation: spin 0.8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

/* Quicks ------------------------------------------------------------------ */
.quicks { display: flex; flex-wrap: wrap; gap: 7px; padding: 10px 18px 0; }
.quick {
  display: inline-flex; align-items: center; gap: 5px;
  padding: 6px 13px; border-radius: var(--r-full); font-size: var(--text-sm); font-weight: 600;
  cursor: pointer; background: var(--surface); border: 1px solid var(--border); color: var(--text-2);
  transition: all var(--dur) var(--ease);
}
.quick .el-icon { font-size: 13px; }
.quick:hover { border-color: var(--brand); color: var(--brand); background: var(--brand-soft); }
...
.composer-box.focused { border-color: var(--brand); box-shadow: var(--ring); }
.composer-box :deep(.el-textarea__inner) {
  background: transparent; border: none; box-shadow: none !important;
  resize: none; padding: 8px 10px; font-size: var(--text-md); line-height: 1.6;
  color: var(--text-1);
}
.composer-foot { display: flex; align-items: center; justify-content: space-between; padding: 2px 6px 2px 12px; }
.hint { font-size: var(--text-xs); color: var(--text-3); }
.send { border-radius: var(--r-sm); }
</style>
