...
- 定制系统 Skill 时完整复制其配置字段和值；不得清空 API Key、服务地址、知识库 ID、超时或更改 TLS 默认值。
- 平台 Skill 凭据存储或当前环境已经为来源 Skill 配置的凭据，按原配置字段物化到第三方 Skill；不存在的凭据不得伪造。
- manifest、报告、提示词和命令输出只声明凭据字段名、来源和 `configured` 状态，不回显值。
