...
9. 将历史结果文件只分配给 `output` 节点。它描述结果契约，不是未来运行批次必须携带的输入。
10. 外部知识库、爬虫、Web 检索和远程 API 使用 `system` 节点；不得以本地文件的 `file_structure`、`table_schema`、`table_process_signal` 或 `field_relationship` 作为该节点角色证据。外部知识的内容范围写在节点描述中，由运行时依据完整规则确定。
...
- 是否误把结果模板同时标成业务输入、外部参考或知识库？外部能力节点是否错误引用了本地文件结构证据？
